export function init() {
    ...
    const respuestasCorrectas = {
        1: "b"  // Pregunta única: Detener la labor, y recordarle que una tarea de alto riesgo requiere ATS y permiso de trabajo (opción b)
    };

    // Total de preguntas actualizado a 1
    const totalPreguntas = 1;
    ...
    function validarPregunta(numeroPregunta) {
        ...
        // Si es la ÚLTIMA pregunta (que es la única)
        if (numeroPregunta === totalPreguntas) {
            ...
            if (esCorrecta) {
                resultadoAlertas.innerHTML = '<div class="alert multi-select-feedback-success"><i class="fas fa-check-circle me-2"></i>Correcto: ¡Muy bien! Tienes la actitud correcta para comenzar este curso de Trabajo seguro en alturas!</div>';
            } else {
                resultadoAlertas.innerHTML =
                    '<div class="alert multi-select-feedback-error"><i class="fas fa-times-circle me-2"></i>Incorrecto: ¡Piénsalo bien! No vale la pena subir, ni tampoco quedarse callado.</div>';
            }
    ...
    function mostrarResumenFinal() {
        ...
        const items = correctnessByQuestion
            .map(({ q, ok }) => {
                ...
                const text = ok ? 'Correcto: ¡Muy bien! Tienes la actitud correcta para comenzar este curso de Trabajo seguro en alturas!' : 'Incorrecto: ¡Piénsalo bien! No vale la pena subir, ni tampoco quedarse callado.';
    ...
    function configurarEventListeners() {
        ...
        // Event listener para botones continuar y finalizar usando delegación de eventos
        document.getElementById('preguntas-container').addEventListener('click', function (event) {
